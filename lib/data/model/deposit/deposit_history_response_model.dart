...
class DepositHistoryListModel {
      ...
      try {
        if (json['detail'] is List<dynamic>) {
          json['detail'].forEach((v) {
            _detail?.add(Details.fromJson(v));
          });
        } else {
          var map = Map.from(json['detail'])
  ...
  List<dynamic>? get detail => _detail;
...
class Gateway {
  Gateway({int? id, String? formId, String? code, String? name, String? alias}) {
    _id = id;
    _formId = formId;
    _code = code;
    _name = name;
