...
import 'package:finovelapp/data/model/deposit/deposit_history_response_model.dart';
...
class DepositController extends GetxController {
  ...
  beforeInitLoadData() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (model.status?.toLowerCase() == 'success') {
        List<DepositHistoryListModel>? tempDepositList =
  ...
  void fetchNewList() async {
    ...
    ResponseModel response =
        await depositRepo.getDepositHistory(page: page, searchText: trx);
    if (response.statusCode == 200) {
      DepositHistoryResponseModel model = DepositHistoryResponseModel.fromJson(
          jsonDecode(response.responseJson));
      if (model.status?.toLowerCase() == 'success') {
