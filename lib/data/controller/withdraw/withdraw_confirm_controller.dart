...
class WithdrawConfirmController extends GetxController {
  ...
  Future<void> getWithdrawConfirmScreenData() async {
    ...
    if (response.statusCode == 200) {
      prev.WithdrawConfirmResponseModel model =
          prev.WithdrawConfirmResponseModel.fromJson(
      ...
      if (model.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        ...
        CustomSnackBar.error(
            errorList: model.message?.error ??
                [MyStrings.withdrawConfirmScreenNotFound]);
      }
    } else {
  ...
  loadData(prev.WithdrawConfirmResponseModel model) async {
    setStatusTrue();
