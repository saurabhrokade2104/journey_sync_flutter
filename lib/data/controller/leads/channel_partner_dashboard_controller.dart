import 'dart:convert';
import 'package:finovelapp/data/model/leads/channel_partner_dashboard_model.dart';
import 'package:finovelapp/data/repo/leads/channel_partner_dashboard_repo.dart';
import 'package:get/get.dart';

import '../../../core/utils/my_strings.dart';
import '../../../views/components/snackbar/show_custom_snackbar.dart';

class ChannelPartnerDashboardController extends GetxController {
   final  ChannelPartnerDashboardRepo channelPartnerDashboardRepo;
  ChannelPartnerDashboardController({required this.channelPartnerDashboardRepo});

  RxBool isLoading = false.obs;
  Rx<ChannelPartnerDashboardModel> partnerDashboardInfo = ChannelPartnerDashboardModel().obs;




Future<void> fetchPartnerDashboard() async {
    isLoading(true);
    try {
      final response = await channelPartnerDashboardRepo.fetchPartnerDashboardInfo();

      print('response check : ${response.responseJson}');

      if (response.statusCode == 200) {
        ChannelPartnerDashboardModel model =
            ChannelPartnerDashboardModel.fromJson(jsonDecode(response.responseJson));

        if (model.status.toString().toLowerCase() == MyStrings.success.toLowerCase()) {
          print('data retrived from summary : ${model.data}');
          partnerDashboardInfo(model);
        } else {
         CustomSnackBar.error(
                errorList: model.message?.error ??
                    [MyStrings.somethingWentWrong.tr]);
          isLoading(false); // Indicate failure due to API-reported error
        }
      } else {
        // Handle non-200 status code as an error
        CustomSnackBar.error(errorList: [MyStrings.somethingWentWrong]);
        print('Error: Non-200 status code received: ${response.statusCode}');
      }
    } catch (e) {
      // Handle any exceptions that occurred during the API call
      CustomSnackBar.error(errorList: [e.toString()]);
      print('Exception caught: $e');
    } finally {
      isLoading(false); // Ensure isLoading is set to false upon completion or failure
      update(); // Notify listeners of state change
    }
  }





}
