...
class EligibilityCheckerRepo {
  ApiClient apiClient;
  EligibilityCheckerRepo({required this.apiClient});


  Future<ResponseModel> submitEligibilityFormData(EligibilityFormModel eligibilityFormModel) async {

    final eligibilityData = eligibilityFormModel.toJson();


    String url = '${UrlContainer.baseUrl}${UrlContainer.submitEligibilityFormDetals}';
    ResponseModel model =
        await apiClient.request(url, Method.postMethod, eligibilityData, passHeader: true);
    return model;
  }
...
Future<ResponseModel> verifyOtpPhone(String mobileNumber, String otp) async {
  ...
  Future<dynamic> verifyPanNumber(String panNumber, String token, String fullName, String dob) async {
  var headers = {
    'Authorization': token,
    'accept': 'application/json',
    'content-type': 'application/json',
  ...
  try {
    var response = await http.post(
      Uri.parse('https://api.sandbox.co.in/kyc/pan/verify'),
      headers: headers,
      body: body,
    );
    debugPrint('Response status: ${response.body}');
