...
class SmsEmailVerificationRepo {
  ...
  Future<ResponseModel> verifyLoginMobileOtp({required String phone , required String otp}) async {
    final map = {
      'mobile':phone,
      'otp': otp,
    };
    ...
    String url =
        '${UrlContainer.baseUrl}${UrlContainer.verifyLoginSmsEndPoint}';
    ResponseModel responseModel =
        await apiClient.request(url, Method.postMethod, map, passHeader: false);
        print('check data : ${responseModel.responseJson}');
    return responseModel;
  }
  ...
  Future<bool> resendLoginVerifyCode({required String phoneNumber}) async {

    String url = '${UrlContainer.baseUrl}${UrlContainer.loginWithMobile}';
    Map<String, String> map = {"mobile": phoneNumber};
    ResponseModel response =
        await apiClient.request(url, Method.postMethod, map, passHeader: false);
