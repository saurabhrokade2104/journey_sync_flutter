...
class LanguageDialogBody extends StatefulWidget {
  ...
  const LanguageDialogBody(
      {super.key, required this.langList, this.fromSplashScreen = false});
                  ...
                  itemBuilder: (context, index) {
                    return GestureDetector(
                      onTap: () async {
                        ...
                        String languageCode =
                            widget.langList[index].languageCode;
                        final repo =
                        ...
                        ResponseModel response =
                            await repo.getLanguage(languageCode);
                        if (response.statusCode == 200) {
                          try {
                            ...
                            var value = resJson['data']['language_data']
                            ...
                            language[
                                    '${widget.langList[index].languageCode}_${'US'}'] =
                                json;

                            Get.clearTranslations();
                            Get.addTranslations(
                                Messages(languages: language).keys);

                            Locale local = Locale(
                                widget.langList[index].languageCode, 'US');
